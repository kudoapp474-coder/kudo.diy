...
import { loadWorkspacePermissions, normalizePermissions } from "../../../../lib/permissions";
...
export async function PATCH(request: Request) {
  ...
  const existing = await loadWorkspacePermissions(auth.db, auth.workspaceId);
  const merged = normalizePermissions({ ...existing, ...body.permissions });
