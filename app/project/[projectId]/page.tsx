...
export default async function ProjectPage({ params, searchParams }: { params: Promise<{ projectId: string }>; searchParams: Promise<{ task?: string; autorun?: string; notice?: string; model?: string }> }) {
  ...
  return <ProjectWorkspace projectId={projectId} initialTask={query.task?.slice(0, 12000) ?? ""} autoRun={query.autorun === "1"} initialNotice={query.notice?.slice(0, 500) ?? ""} initialModel={query.model?.slice(0, 80) ?? ""} />;
}
